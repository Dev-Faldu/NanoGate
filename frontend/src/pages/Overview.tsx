      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.55fr_1fr]">
        <Card>
          ...
          {recent.isLoading ? <StateView kind="loading" /> : rows.length === 0 ? (
            <StateView kind="empty" title="No requests yet" detail="Use the composer to send a real request through the gateway, or point an OpenAI SDK at /v1." />
          ) : (
        ...
        <Card>
          <CardHeader eyebrow="Try it" title="Send a real request" subtitle="Runs the full pipeline on the ZGX Nano. Nothing here is simulated." />
          <div className="p-6"><Composer onResult={(r) => r.headers["x-nanogate-receipt-id"] && setFocusReceipt(r.headers["x-nanogate-receipt-id"])} /></div>
        </Card>
      </div>
